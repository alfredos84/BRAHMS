...
- Python 3.10+
- Linux: a C++ compiler (g++), FFTW3, HDF5, nlohmann/json — installed automatically by `install/setup_linux.sh`.
...
This installs system dependencies, creates a Python virtual environment, builds the CPU simulation engine (and the GPU engine too, if an NVIDIA GPU + CUDA toolkit is detected), and adds a **BRAHMS** launcher to your applications menu and Desktop — just double-click it to open the app.
...
This creates a Python virtual environment and a **BRAHMS** shortcut on your Desktop and Start Menu. Building the C++ simulation engines on native Windows needs an extra manual step (MSYS2/MinGW for the CPU engine, CUDA Toolkit + MSVC for the GPU engine) — see the comments at the top of `install/setup_windows.ps1` for exact commands, or use WSL and run the Linux installer instead.
